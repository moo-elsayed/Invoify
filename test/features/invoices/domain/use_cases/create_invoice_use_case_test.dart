...
void main() {
  ...
  setUp(() {
    ...
    sut = CreateInvoiceUseCase(mockRepo);
  });
  ...
  const tFailure = ServerFailure(error: 'Failed to create invoice');

  test(
    'should return NetworkSuccess<InvoiceEntity> when call succeeds',
    () async {
      when(
        () => mockRepo.createInvoice(invoice: tInvoiceEntity),
      ...
      verify(() => mockRepo.createInvoice(invoice: tInvoiceEntity)).called(1);
      verifyNoMoreInteractions(mockRepo);
    },
  );

  test('should return NetworkFailure<InvoiceEntity> when call fails', () async {
    when(
      () => mockRepo.createInvoice(invoice: tInvoiceEntity),
    ...
    expect((result as NetworkFailure<InvoiceEntity>).failure, equals(tFailure));
    verify(() => mockRepo.createInvoice(invoice: tInvoiceEntity)).called(1);
    verifyNoMoreInteractions(mockRepo);
  });
}
